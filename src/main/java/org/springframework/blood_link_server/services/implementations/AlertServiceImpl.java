...
public class AlertServiceImpl implements AlertService {
    ...
    public Alert createSendAlert(BloodRequest request) {
        ...
        Alert alert1 = alertRepository.saveAndFlush(alert);

        List<Donor> compatibleDonors = request.getBloodBank().getAffiliatedDonors().stream().filter(donor -> alert1.getCompatibleTypes().contains(donor.getBloodType())).toList();


        compatibleDonors.forEach(donor ->
                notificationRepository.save(Notification.builder().alert(alert1).donor(donor).alertStatus(alert1.getStatus()).build())
        );
